...
class NearController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //求职信息
        let jobInfo=UIButton(frame: CGRectMake(0, 63, width/3, width/3))
        jobInfo.tag=1
        jobInfo.addTarget(self, action:Selector("ButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
        let jobInfoIco=UIImageView(frame: CGRectMake(width/9, width/12+63, width/9, width/9))
        jobInfoIco.image=UIImage(named: "求职信息.png")
        let jobInfoLabel=UILabel(frame: CGRectMake(width/12, width/36*7+63, width/6, width/12))
        ...
        //特色推荐
        let specRecommand=UIButton(frame: CGRectMake(width/3, 63, width/3, width/3))
        specRecommand.tag=2
        specRecommand.addTarget(self, action:Selector("ButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
        let SRIco=UIImageView(frame: CGRectMake(width/3+width/9, width/12+63, width/9, width/9))
        SRIco.image=UIImage(named: "特色推荐.png")
        let SRLabel=UILabel(frame: CGRectMake(width/3+width/12, width/36*7+63, width/6, width/12))
        SRLabel.text="特色推荐"
        ...
        //爱好交友
        let hobby=UIButton(frame: CGRectMake(width/3*2, 63, width/3, width/3))
        hobby.tag=3
        hobby.addTarget(self, action:Selector("ButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
        let hobbyIco=UIImageView(frame: CGRectMake(width/3*2+width/9, width/12+63, width/9, width/9))
        hobbyIco.image=UIImage(named: "爱好交友.png")
        let hobbyLabel=UILabel(frame: CGRectMake(width/3*2+width/12, width/36*7+63, width/6, width/12))
        ...
        //其他
        let other=UIButton(frame: CGRectMake(0, 63+width/3, width/3, width/3))
        other.tag=4
        other.addTarget(self, action:Selector("ButtonAction:"), forControlEvents: UIControlEvents.TouchUpInside)
        let otherIco=UIImageView(frame: CGRectMake(width/9, width/12+63+width/3, width/9, width/9))
        otherIco.image=UIImage(named: "其他.png")
        let otherLabel=UILabel(frame: CGRectMake(width/8, width/36*7+63+width/3, width/12, width/12))
        otherLabel.text="其他"
