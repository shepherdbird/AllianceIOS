...
class OneChargeDetailController: UITableViewController ,UIPickerViewDelegate, UIPickerViewDataSource{

    @IBOutlet var Detail: UITableView!
    var GrabCommodityId:Int=0
    var Phone:String=""
    ...
    func connect(){
        do {
            let params:Dictionary<String,AnyObject>=["grabcommodityid":GrabCommodityId,"phone":"2"]
    ...
    func LiMaCanYu_OK(){
        do {
            let params:Dictionary<String,AnyObject>=["grabcommodityid":GrabCommodityId,"phone":"2","count":pk.selectedRowInComponent(0),"type":1]
            let opt=try HTTP.POST(URL+"/grabcommodities/buy", parameters: params)
            opt.start { response in
                if let err = response.error {
                    print("error: \(err.localizedDescription)")
                    return //also notify app of failure as needed
    ...
    func BuyAll(){
        do {
            let params:Dictionary<String,AnyObject>=["grabcommodityid":GrabCommodityId,"phone":"2","type":1]
            let opt=try HTTP.POST(URL+"/grabcommodities/buyall", parameters: params)
            opt.start { response in
                if let err = response.error {
                    print("error: \(err.localizedDescription)")
                    return //also notify app of failure as needed
    ...
    func FirstCell()->UITableViewCell {
        ...
        if(OneChargeView!.detail.islotteried==0){
    ...
    func ButtonAction(sender:UIButton){
        if(sender.tag==1){
            ...
            OK.setTitle("一元夺宝", forState: UIControlState.Normal)
