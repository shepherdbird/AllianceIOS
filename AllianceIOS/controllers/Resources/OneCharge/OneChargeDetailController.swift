...
class OneChargeDetailController: UITableViewController ,UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    func connect(){
        do {
            let params:Dictionary<String,AnyObject>=["grabcommodityid":GrabCommodityId,"phone":Phone]
    ...
    func ButtonAction(sender:UIButton){
        if(sender.tag==1){
            ...
            let reqAction=UIAlertAction(title: "确定", style: UIAlertActionStyle.Default) { (action:UIAlertAction) -> Void in
                let another=Check()
                another.Id=self.OneChargeView!.detail.id
                another.All=0
                another.Through="commodity"
                another.Count=Int(self.pk.selectedRowInComponent(0))+1
                another.SetTitle=self.OneChargeView!.detail.title
            ...
            let another=Check()
            another.Id=self.OneChargeView!.detail.id
            another.Count=self.OneChargeView!.detail.needed
            another.Through="commodity"
            another.SetTitle=self.OneChargeView!.detail.title
            another.All=1
            self.navigationController?.pushViewController(another, animated: true)
