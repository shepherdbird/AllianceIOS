...
class SNS: UITableViewController {
    ...
    override func tableView(tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        if((MessageList) != nil){
            if(section==self.MessageList!.items.count-1){
                return 200
            }
            return 0.01
        }
    ...
    override func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        if((MessageList) != nil){
            if(indexPath.row==0){
                ...
                title.text=MessageList!.items[indexPath.section].title
                title.font=UIFont.systemFontOfSize(18)
                title.numberOfLines=0
                print(MessageList!.items[indexPath.section].title)
    ...
    func First(row:Int)->UITableViewCell{
        ...
        let title=UILabel(frame: CGRectMake(60,50,self.view.frame.width-120,20))
        title.text=MessageList!.items[row].title
        title.font=UIFont.systemFontOfSize(18)
        title.numberOfLines=0
        boundingRect=GetBounds(self.view.frame.width-80, height: 300, font: title.font, str: title.text!)
    ...
    func Second(row:Int)->UITableViewCell{
        let cell=UITableViewCell()
        if MessageList!.items[row].isconcerned=="1" {
        ...
        if MessageList!.items[row].isliked=="1" {
        ...
        likeCount.text=MessageList!.items[row].likecount
        ...
        commentCount.text=MessageList!.items[row].replycount
    ...
    func Concern(sender:UIButton){
        do {
            let params:Dictionary<String,AnyObject>=["myphone":Phone,"concernphone":MessageList!.items[sender.tag-10000].phone]
    ...
    func Like(sender:UIButton){
        ...
        if(sender.tag%2==0){
            do {
                let params:Dictionary<String,AnyObject>=["phone":Phone,"tbmessageid":MessageList!.items[sender.tag/2-2].id]
            ...
            do {
                let params:Dictionary<String,AnyObject>=["phone":Phone,"tbmessageid":MessageList!.items[(sender.tag-5)/2].id]
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        anotherView.TbMessageId=Int(self.MessageList!.items[indexPath.section].id)!
        print("messageid: "+self.MessageList!.items[indexPath.section].id)
