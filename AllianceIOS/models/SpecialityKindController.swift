//
//  SpecialityKindController.swift
//  AllianceIOS
//
...
class SpecialityKindController: UITableViewController {
    ...
    var info:Info?
        ...
        didSet
        {
            ...
            for(var i=0;i<self.info?.items.count;i++){
                Allkind.append((self.info?.items[i].kind)!)
            }
    ...
    var Allkind: Array<String> = []
    var selected:String?
    var selectedIndex:Int? = nil
    var Sepcreq:SpecialityReqController?
    ...
    override func viewDidLoad() {
        ...
        if let game = selected {
            selectedIndex = Allkind.indexOf(game)
        }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = Allkind[indexPath.row]
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        selectedIndex = indexPath.row
         ...
         Sepcreq?.spkind?.text = self.Allkind[indexPath.row]
            
            //update the checkmark for the current row
        let cell = tableView.cellForRowAtIndexPath(indexPath)
        cell?.accessoryType = .Checkmark
        Sepcreq?.tableView.reloadData()
    ...
    func connect(){
        print("ccccc")
        do {
            let opt=try HTTP.GET("http://183.129.190.82:50001/v1/kindsofrecommendation/list")
